// Example: Basic Read & Write for I2Connect: EEPROM
// Last Update: June 21, 2026
// Support: http://ptsolns.com/contact-us
//
// WHAT THIS EXAMPLE DOES
// Writes a single byte to EEPROM memory and then reads it back.
// Demonstrates the most basic EEPROM storage and retrieval functions.
//
// SKILLS LEARNED
// writeByte()
// readByte()
...
const uint32_t BYTE_ADDRESS   = 0;      // EEPROM memory location
const uint8_t  VALUE_TO_WRITE = 123;    // Value to store. Valid range: 0 to 255.
...
void setup() {
    ...
    if (!eeprom.writeByte(BYTE_ADDRESS, VALUE_TO_WRITE)) {
    ...
    if (!eeprom.readByte(BYTE_ADDRESS, valueRead)) {
