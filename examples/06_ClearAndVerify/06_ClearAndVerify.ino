// Example: Clear and Verify for I2Connect: EEPROM
// Last Update: June 21, 2026
// Support: http://ptsolns.com/contact-us
//
// WHAT THIS EXAMPLE DOES
// Clears a section of EEPROM memory and verifies the result.
//
// SKILLS LEARNED
// clear()
// fill()
...
const uint32_t CLEAR_START_ADDRESS = 3000;
const uint32_t CLEAR_LENGTH_BYTES  = 128;
...
void setup() {
    ...
    if (!eeprom.clear(CLEAR_START_ADDRESS, CLEAR_LENGTH_BYTES)) {
    ...
    for (uint32_t i = 0; i < CLEAR_LENGTH_BYTES; i++) {
        uint8_t value = 0;

        if (!eeprom.readByte(CLEAR_START_ADDRESS + i, value)) {
            verified = false;
            break;
        }
    ...
    Serial.println(verified ? "Passed" : "Failed");
}
